...
import { ColorSelector } from "@/components/- ReUseable/FORM";

export function FORM_Of_Logo_Designing() {
  const [selectedColors, setSelectedColors] = React.useState<string[]>([]);
  const [selectedIndustry, setSelectedIndustry] = React.useState<string>("");
  const [selectedLogoStyle, setSelectedLogoStyle] = React.useState<string>("");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/designing/form/logo-designing/submit`,
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dropdown for LogoStyle Selection */}
        <LabelInputContainer className="mb-8">
          <Label htmlFor="industry">Logo Style</Label>
          <select
            id="logoStyle"
            name="logoStyle"
            className="w-full rounded-md border border-neutral-300 bg-white p-3 text-neutral-700 shadow-sm 
          ...
          >
            ...
            {logoStyleOptions.map((logoStyle, index) => (
              <option key={index} value={logoStyle}>
                {logoStyle}
              </option>
            ))}
          </select>
        ...
        {/* <div className="mb-8 flex flex-col space-y-2 md:flex-row md:space-y-0">
          ...
          <div className="ml-10 sm:ml-6 lg:ml-10 grid lg:grid-cols-6 sm:grid-cols-3 md:grid-cols-3 gap-x-6 gap-y-3">
